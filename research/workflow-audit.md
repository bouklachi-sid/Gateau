...
Une vérification de Messenger n’a révélé aucune réponse d’administrateur exploitable portant une autorisation de partage. L’accès à certains historiques chiffrés requiert la restauration de messages, étape qui n’a pas été contournée. Il n’existe donc à ce stade aucun espace tiers autorisé pour cette publication.

Le groupe public local `nass kolea` (1,2 K membres affichés) a été rouvert et son seul administrateur, `Nacif Gx`, a été confirmé dans la liste des membres. Après confirmation du propriétaire pour lui adresser une demande d’autorisation individualisée, Facebook a indiqué que ce compte n’accepte pas de messages de notre part. Aucun message n’a été envoyé et aucun partage n’a été tenté ; la piste est donc bloquée jusqu’à ce qu’un canal de contact public et légitime soit disponible.

Le statut public d’un groupe ou d’une Page ne constitue pas une autorisation de diffusion commerciale. Les prochaines actions de prospection resteront donc limitées à l’identification de communautés locales et à des demandes d’accord individualisées ; aucun partage tiers ne sera réalisé sans réponse positive explicite d’un administrateur.

## Optimisation de suivi automatique — 24 août 2026

Le tableau de pilotage affiche désormais un compteur **« Prêts auto. »** et, pour chaque contenu encore `scheduled` ou `publishing`, un état calculé à partir des mêmes règles que le runner serveur. Un contenu ne peut être affiché comme prêt que s’il possède un média au statut persistant `retouched`, une validation manuelle, la confirmation de préservation du gâteau, une scène renseignée et distincte, ainsi que l’appel visible exact `للطلبات: 0555188455`. Les contenus `published`, `cancelled` ou autres éléments historiques portent maintenant le libellé **« Historique — non planifié »** : ils ne sont jamais présentés comme une nouvelle publication à envoyer.

La capture de contrôle de l’interface confirme que l’automatisation est active, que la connexion Meta est indiquée **OK**, qu’aucun contenu futur n’est actuellement prêt, et que les contenus non conformes expliquent le correctif requis. La vérification TypeScript et Vitest est réussie avec **10 fichiers et 27 tests**, dont deux cas dédiés à l’état de préparation automatique.

Le dernier jeton Meta fourni a été enregistré uniquement dans le secret serveur `META_PAGE_ACCESS_TOKEN` et a passé les trois validations légères existantes : identité Graph, lecture de la Page et contrôle live sans exposition du jeton. Le job propriétaire actif `facebook-patisserie-owner-1` reste activé toutes les cinq minutes ; les dernières exécutions consultées renvoient HTTP 200 avec `{"ok":true,"processed":0}`. Cette dernière valeur signifie qu’aucun contenu éligible n’a été envoyé pendant ces cycles. La preuve de reprise de bout en bout demeure donc ouverte : elle exige un contenu image réellement approuvé et dû, suivi d’un passage Heartbeat avec `processed > 0`, un statut `published` et un `metaPostId` enregistré. Aucun contenu déjà publié, notamment `30001`, `30002` ou `60001`, ne sera renvoyé pour produire cette preuve.
